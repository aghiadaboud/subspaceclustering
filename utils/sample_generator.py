...
def generate_sample(n = 0, d = 0, subspace_clusters = None):
  ...
  sample = np.random.uniform(-100, 100, size=(n,d))
  labels = np.zeros((n, d))
  cluster_label = 1
  for cluster in subspace_clusters:
    ...
    sub_d = cluster[3]
    std = cluster[4]
    sub_sample, l = make_blobs(n_samples=sub_n, n_features=sub_d, centers=1, cluster_std=std, center_box=(-100, 100), shuffle=True, random_state=None)
