...
def overall_f1_recall(true_clusters, found_clusters):
  ...
  for subspace in true_clusters.keys():
    for cluster in true_clusters.get(subspace):
      intersections = np.array(list(map(lambda x: len(np.intersect1d(x, cluster)), all_found_clusters)))
  ...
  for cluster in all_found_clusters:
    ...
    if np.any(coverages):
      ...
      for best_matching_cluster in best_matching_clusters:
        mapped_clusters[best_matching_cluster] = np.union1d(mapped_clusters[best_matching_cluster], cluster)
...
def overall_rnia(db_n, db_d, true_clusters, found_clusters):
  ...
  true_micro_objects, found_micro_objects = compute_true_and_found_micro_objects(db_n, db_d, true_clusters, found_clusters)
  cardinality_intersection_micro_objects = np.minimum(true_micro_objects, found_micro_objects).sum()
  cardinality_union_micro_objects = np.maximum(true_micro_objects, found_micro_objects).sum()
  rnia = (cardinality_union_micro_objects - cardinality_intersection_micro_objects) / cardinality_union_micro_objects
  return 1 - rnia
...
def overall_ce(db_n, db_d, true_clusters, found_clusters):
  ...
  cost_matrix = np.zeros(shape = (max_cluster_number,max_cluster_number)) #matching matrix
  i= 0
  j= 0
    ...
    for cluster_t in true_clusters.get(subspace_t):
      for subspace_r in found_clusters.keys():
        ...
        if mutual_dimensions == 0:
          ...
          intersections_with_cluster_t = list(map(lambda x: len(np.intersect1d(x, cluster_t)), found_clusters.get(subspace_r)))
          for intersection in intersections_with_cluster_t:
            cost_matrix[i][j] = intersection * mutual_dimensions #micro objects intersection
            j = j + 1
      i = i + 1
      j = 0
  row_ind, col_ind = linear_sum_assignment(cost_matrix, maximize=True) #find a permutation of clusters that maximaize the total sum over all cardinalities
...
def compute_f1_sc_clus(p, q):
  ...
  for subspace_p in p.keys():
    relevant_subspaces = [s for s in q.keys() if len(np.intersect1d(subspace_p, s)) != 0]
    for cluster_p in p.get(subspace_p):
      ...
      for subspace_q in relevant_subspaces:
        ...
        intersections_with_cluster_p = np.array(list(map(lambda x: len(np.intersect1d(x, cluster_p)),clusters_in_subspace_q)))
        micro_objects_intersections = intersections_with_cluster_p * mutual_dimensions
        recalls_sc = micro_objects_intersections / micro_objects_cluster_p
        if isinstance(subspace_q, int):
          precisions_sc =  micro_objects_intersections / list(map(lambda x: len(x),clusters_in_subspace_q))
        else:
          precisions_sc =  micro_objects_intersections / list(map(lambda x: len(x)*len(subspace_q),clusters_in_subspace_q))
        with np.errstate(divide='ignore',invalid='ignore'):
          f1_sc = (2 * recalls_sc * precisions_sc) / (recalls_sc + precisions_sc)
