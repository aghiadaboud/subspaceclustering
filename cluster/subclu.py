            ...
            for cand in cand_S_k_plus_1:
                ...
                for cluster in self.__clusters.get(bestSubspace, []):
                    ...
                    neigh = NearestNeighbors(
                        n_neighbors=1, radius=self.__eps, metric="euclidean", n_jobs=-1
                    )
                    neigh.fit(points)
                    distances = neigh.radius_neighbors_graph(
                        points, mode="distance", sort_results=True
                    )
                    ...
                    self.__noise.get(cand).append(
                        cluster[list(np.where(labels == -1)[0])]
                    )
