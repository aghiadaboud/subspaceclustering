...
def cluster_10d_xn_2sc():
  ...
  for i,n in enumerate(size):
    ...
    clustering_method = Clustering_By_dbscan(fires_dbscan_params[i][0], fires_dbscan_params[i][1])
...
def cluster_xd_1000n_1sc():
  ...
  for i, d in enumerate(dimensionalities):
    samples, labels = generate_samples(1600, d, 1.5, 1, [[range(700, 850), range(d-7, d), 0.2]])
    if i < 4:
...
def cluster_xd_xn_10sc():
  ...
  for i, dataset in enumerate(datasets):
    ...
    samples = samples[:, :-1]
    if i < 4:
      subclu_instance = subclu(samples, subclu_dbscan_params[i][0], subclu_dbscan_params[i][1])
      start_subclu = time.perf_counter()
      subclu_instance.process()
      stop_subclu = time.perf_counter()
      subclu_runtime.append(stop_subclu - start_subclu)
