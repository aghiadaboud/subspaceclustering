    ...
    def process(self, data, new_eps=None):
        ...
        if new_eps is None:
            dbscan_instance = DBSCAN(
                eps=self.__eps,
                min_samples=self.__minpts,
                metric=self.__distance_metric,
                n_jobs=-1,
            ).fit(data)
        else:
            neigh = NearestNeighbors(
                n_neighbors=1, radius=new_eps, metric="euclidean", n_jobs=-1
            )
            neigh.fit(data)
            distances = neigh.radius_neighbors_graph(
                data, mode="distance", sort_results=True
            )
            dbscan_instance = DBSCAN(
                eps=new_eps, min_samples=self.__minpts, metric="precomputed", n_jobs=-1
            ).fit(distances)
...
class Clustering_By_kmeans(Clustering_Method):
    ...
    def process(self, data):
        ...
        kmeans_instance = KMeans(
            n_clusters=self.__amount_centers,
            init="k-means++",
            n_init=10,
            max_iter=300,
            tol=0.0001,
...
class fires:
    ...
    def __verify_arguments(self):
        ...
        if not isinstance(
            self.__clustering_method,
            (Clustering_By_dbscan, Clustering_By_kmeans, Clustering_By_clique),
        ):
            raise ValueError(
                "clustering_method should be instance of Clustering_By_dbscan or Clustering_By_kmeans or Clustering_By_clique."
            )
        ...
        while not all_clusters_checked:
            ...
            for i, cluster in enumerate(self.__pruned_C1):
                ...
                intersection = np.intersect1d(
                    self.__pruned_C1[most_similar_cluster], cluster
                )
                difference = np.setdiff1d(
                    cluster, self.__pruned_C1[most_similar_cluster]
                )
                if (
                    len(intersection) >= two_thirds_of_s_avg
                    and len(difference) >= two_thirds_of_s_avg
                ):
                    all_clusters_checked = False
            ...
            for j, new_clusters in splits.items():
                ...
                self.__baseCluster_to_dimension[
                    len(self.__pruned_C1) - 1
                ] = self.__baseCluster_to_dimension.get(j)
    ...
    def compute_most_similar_cluster(self, cluster_index, cluster):
        ...
        intersections = list(
            map(lambda x: len(np.intersect1d(x, cluster)), self.__pruned_C1)
        )
    ...
    def compute_k_most_similar_clusters(self):
        ...
        if self.__k < len(self.__pruned_C1):
            for i, cluster in enumerate(self.__pruned_C1):
                ...
                intersections[i] = -1
                self.__k_most_similar_clusters[i] = np.argpartition(
                    intersections, -self.__k
                )[-self.__k :]
        else:
            raise ValueError(
                "k should not be equal or greater than the number of base clusters('%d')"
                % len(self.__pruned_C1)
            )
    ...
    def compute_best_merge_candidates(self):
        ...
        for combination in sorted_combinations:
            if (
                len(
                    np.intersect1d(
                        self.__k_most_similar_clusters.get(combination[0]),
                        self.__k_most_similar_clusters.get(combination[1]),
                    )
                )
                >= self.__mu
            ):
                self.__best_merge_candidates[
                    combination[0]
                ] = self.__best_merge_candidates.get(combination[0], [])
                self.__best_merge_candidates.get(combination[0]).append(combination[1])
                self.__best_merge_candidates[
                    combination[1]
                ] = self.__best_merge_candidates.get(combination[1], [])
    ...
    def generate_subspace_cluster_approximations(self):
        ...
        for combination in sorted_combinations:
            if combination[0] in self.__best_merge_candidates.get(
                combination[1]
                ...
                if all(
                    Counter(x) != Counter(merged_list)
                    for x in self.__subspace_cluster_approximations
                ):
    ...
    def prune_irrelevant_merge_candidate_of_subspace_cluster_approximations(self):
        ...
        for index, approximation in enumerate(self.__subspace_cluster_approximations):
            score_approximation = (
                self.compute_quality_of_subspace_cluster_approximation(approximation)
            )
            score_approximation_ignoring_one_cluster_eachTime = (
                self.compute_quality_of_approximation_ignoring_one_cluster_eachTime(
                    approximation
                )
            )
            if np.any(
                ...
                while i < len(approximation) and len(approximation) > 1:
                    if len(approximation) < count_remaining_base_clusters:
                        score_approximation_ignoring_one_cluster_eachTime = self.compute_quality_of_approximation_ignoring_one_cluster_eachTime(
                            approximation
                        )
                        count_remaining_base_clusters = (
                            count_remaining_base_clusters - 1
                        )
                    ...
                    if score_approximation_ignoring_one_cluster_eachTime[
                        i
                    ] > score_approximation and np.all(
                        np.less_equal(
                            remaining_scores,
                            score_approximation_ignoring_one_cluster_eachTime[i],
                        )
                        == True
                    ):
                        approximation.pop(i)
        ...
        for j in range(amount_approximations - 1):
            if j not in must_deleted:
                ...
                for combi in [w for w in sorted_combinations if w[0] == j]:
                    if set(jth_approximation).issubset(
                        self.__subspace_cluster_approximations[combi[1]]
                    ):
                        must_deleted.add(combi[0])
                        break
                    elif set(self.__subspace_cluster_approximations[combi[1]]).issubset(
                        jth_approximation
                    ):
    ...
    def compute_quality_of_approximation_ignoring_one_cluster_eachTime(
        self, approximation
    ):
        """
        ...
        for i in range(size_approximation):
            approximation_without_one_cluster = np.array(approximation)[
                np.arange(size_approximation) != i
            ]
            score = self.compute_quality_of_subspace_cluster_approximation(
                approximation_without_one_cluster
            )
    ...
    def refine_cluster_approximations(self):
        ...
        for approximation in self.__subspace_cluster_approximations:
            subspace = tuple(
                set(self.__baseCluster_to_dimension.get(k) for k in approximation)
            )
            base_clusters = [self.__pruned_C1[i] for i in approximation]
            base_clusters_union = reduce(np.union1d, base_clusters)
            projections_into_subspace = self.get_points_values_in_subspace(
                base_clusters_union, subspace
            )
            if isinstance(self.__clustering_method, Clustering_By_dbscan):
                new_eps = self.adjust_density_threshold(
                    self.__clustering_method.get_eps(), len(subspace)
                )
                clustering = self.__clustering_method.process(
                    projections_into_subspace, new_eps
                )
            else:
            ...
            if 0 in labels:
                ...
                for label in np.setdiff1d(labels, -1):
                    self.__clusters[subspace].append(
                        base_clusters_union[list(np.where(labels == label)[0])]
                    )
