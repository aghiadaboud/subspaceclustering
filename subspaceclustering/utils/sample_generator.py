...
def generate_samples(n, d, samples_std, seed, subspace_clusters=None):
    ...
    samples = np.random.normal(
        loc=(np.random.uniform(0, 0)), scale=samples_std, size=(n, d)
    )
    _95_percent_of_n = int(n * 95 / 100)
    samples[
        np.ix_(list(range(_95_percent_of_n, n)), list(range(d)))
    ] = np.random.uniform(
        np.min(samples), np.max(samples), size=(n - _95_percent_of_n, d)
    )
    ...
    for cluster in subspace_clusters:
        ...
        for dim in cluster_features:
            ...
            if means_of_dim:
                if len(means_of_dim) % 2 == 0:
                    mean.append(
                        3.5 * std
                        + 3.5 * mean_to_std.get(means_of_dim[-2])
                        + means_of_dim[-2]
                    )
        ...
        labels[
            np.ix_(list(cluster_points), list(cluster_features))
        ] = cluster_label * np.ones((sub_n, sub_d))
